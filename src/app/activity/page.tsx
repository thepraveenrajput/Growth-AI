...
export default function Activity() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto bg-slate-950/20">
      {/* Header */}
      <header className="px-8 py-6 bg-slate-900 border-b border-slate-800 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Verifiable logging of AI copilot queries, tool executions, and merchant actions.</p>
        </div>
      ...
      <section className="p-8 max-w-4xl mx-auto w-full">
        {loading ? (
          ...
          <div className="bg-rose-950/20 border border-rose-900/50 rounded-xl p-6 text-center text-slate-400 max-w-md mx-auto">
            <p className="text-sm mb-4">{error}</p>
            <button onClick={fetchActivity} className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded text-xs text-white">Retry</button>
          </div>
        ) : activities.length === 0 ? (
          <div className="bg-slate-900 border border-slate-850 rounded-xl p-12 text-center text-slate-500 max-w-md mx-auto">
                ...
                <div key={a.id} className="relative">
                  ...
                  <span className={`absolute -left-[35px] top-1.5 flex items-center justify-center w-6 h-6 rounded-full border ${
                    isAction
                      ...
                      : 'bg-slate-900 text-violet-400 border-slate-850'
                  }`}>
                    {isAction ? (
                  ...
                  {/* Log Content Card */}
                  <div className="bg-slate-900 border border-slate-850 rounded-xl p-5 shadow-lg">
                    {/* Timestamp */}
                    ...
                    {/* Action Meta Details */}
                    {isAction && (
                      <div className="text-[10px] text-slate-400 bg-slate-950 rounded-lg p-2.5 border border-slate-850">
                        <span className="font-semibold block mb-0.5 text-slate-500">Merchant Response Feedback:</span>
                        "{a.meta.feedback}"
                      </div>
                    )}
                    ...
                    {!isAction && a.meta.toolCalls && a.meta.toolCalls.length > 0 && (
                      <div className="space-y-2 mt-3 pt-3 border-t border-slate-850">
                        <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Tools Executed:</span>
                        <div className="flex flex-wrap gap-2">
                          {a.meta.toolCalls.map((tc: any, tcIdx: number) => (
                            <span 
                              key={tcIdx}
                              className="inline-flex items-center gap-1 text-[9px] font-mono px-2 py-0.5 rounded bg-slate-950 text-slate-300 border border-slate-800"
                              title={`Input Params: ${JSON.stringify(tc.params)}`}
                            >
                              <Terminal className="w-2.5 h-2.5 text-violet-400" /> {tc.toolName}
                            </span>
                          ))}
                        </div>
                      </div>
